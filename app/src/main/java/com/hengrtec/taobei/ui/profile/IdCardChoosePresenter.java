 ...
 */
public class IdCardChoosePresenter {
  ...
  private static final String AVATAR_FOLDER = "/id_card/";
  ...
  public IdCardChoosePresenter(Activity context) {
  ...
  public void showChooseDialog() {
    ...
    if (null == mChooseDialog) {
      mChooseDialog = new AlertDialog.Builder(mContext).setItems(items, new DialogInterface
          .OnClickListener() {
        @Override
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    switch (requestCode) {
      case IdCardChoosePresenter.REQUEST_CODE_CAPTURE:
        break;
      case IdCardChoosePresenter.REQUEST_CODE_ALBUM:
    ...
    Subscription subscription = mAppService.upload(body).subscribeOn(Schedulers.io()).observeOn
        ...
        .subscribe(new UiRpcSubscriber<String>(mContext) {
          ...
          protected void onSuccess(String path) {
            ((CustomApp) mContext.getApplicationContext()).getGlobalComponent().getGlobalBus().post(new UploadIdCardEvent(path));
          }
  ...
  public class UploadIdCardEvent {
    ...
    public UploadIdCardEvent(String path) {
      this.path = path;
    }
  }
}
