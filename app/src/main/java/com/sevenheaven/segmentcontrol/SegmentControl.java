...
public class SegmentControl extends View {
  ...
  public SegmentControl(Context context, AttributeSet attrs, int defStyle) {
    ...
    mTextNormalColor = ta.getColor(R.styleable.SegmentControl_textColorNormal, 0x000000);
    mTextSelectedColor = ta.getColor(R.styleable.SegmentControl_textColorSelected, 0xFFFFFF);
    ...
    if (mColors == null) {
      mColors = new ColorStateList(new int[][]{{}}, new int[]{0xFBDB00});
    }
  ...
  public void onDraw(Canvas canvas) {
    ...
    if (mTexts != null && mTexts.length > 0) {
      for (int i = 0; i < mTexts.length; i++) {
        ...
        //draw texts
        canvas.drawText(mTexts[i], mCacheBounds[i].left + (mSingleChildWidth - mTextBounds[i].width()) / 2, mCacheBounds[i].top + ((mSingleChildHeight + mTextBounds[i].height()) / 2 - 4), mPaint);
      }
    }
